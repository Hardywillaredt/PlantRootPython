...
class MetaGraphThread(QThread):
    ...
    '''
    updateMetaGraph = pyqtSignal(object)
        ...
        while True:
            if not self.fileIsLoaded:
                if self.loadFile:
                    ...
                    print(self.fileToLoad)
#                    self.printToMain.emit(self.fileToLoad)
#                    self.graph.initializeFromSkeleton()
                    print("graph loaded from skeleton")
#                    self.printToMain.emit("graph loaded from skeleton")
...
#       and set the node size map to the greater of its current size or the edge thickness for each edge node
    ...
    def makeMetaGraphGL(self):
        ...
        gl.glEndList() 
        self.currentGL.emit(self.metaGL)
        return
